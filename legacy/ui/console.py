...
def criar_tabela_documentos(documentos, total, pagina, por_pagina=20):
    ...
    for doc in documentos:
        ...
        table.add_row(str(doc[0]), centro, doc[3] or "N/D", titulo, tamanho_str)
...
def mostrar_documento(doc):
    ...
    # Rodapé
    console.print(
        "[dim]⏎ Enter para voltar | [yellow]e[/yellow] para exportar | [green]t[/green] para traduzir[/dim]"
    )
...
def spinner_processo(mensagem, funcao, *args, **kwargs):
    ...
    with Progress(
        ...
        task = progress.add_task(description=f"[bold cyan]{mensagem}[/bold cyan]", total=None)
