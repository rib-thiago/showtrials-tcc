...
TIPOS_DOCUMENTO = {
    ...
    "acusacao": {
        "padroes": ["Проект обвинительного заключения", "Обвинительное заключение"],
        "prioridade": 3,
    ...
    "carta": {"padroes": ["Письмо"], "prioridade": 5, "descricao": "Correspondência"},
    "relatorio": {
    ...
    "depoimento": {
        "padroes": ["Показания", "Показание"],  # plural (já existe)  # singular (FALTANDO!)
        "prioridade": 7,
    ...
    for row in tabela.find_all("tr"):
        ...
        if len(colunas) >= 2:
            ...
            if link_tag:
                ...
                links.append({"titulo": titulo, "url": href, "data_original": data_raw})
...
def montar_documento(centro, meta, texto):
    ...
    documento = {
        ...
        "envolvidos": (
            ", ".join(classificacao["envolvidos"]) if classificacao["envolvidos"] else None
        ),
