...
from src.interface.cli.commands_analise import ComandoAnalisarAcervo, ComandoAnalisarDocumento
...
from src.interface.cli.commands_traducao import ComandoAlternarIdioma, ComandoTraduzir
from src.interface.cli.menu import MenuCentro, MenuPrincipal
from src.interface.console import cabecalho, console, limpar_tela, mostrar_erro, mostrar_sucesso
...
class ShowTrialsApp:
    ...
    def __init__(self):
        ...
        self.tradutor_persistente = TradutorComPersistenciaAdapter(
            self.tradutor_service, self.repo_traducao
        )
        ...
        self.obter_use_case = ObterDocumento(self.repo, self.repo_traducao).com_traducao_nomes(True)
        ...
        self.analisar_acervo_use_case = AnalisarAcervo(
            repo_doc=self.repo, registry=self.registry  # ← Corrigido também
        )
        ...
        self.cmd_traduzir = ComandoTraduzir(self.traduzir_use_case, self.listar_traducoes_use_case)
        self.cmd_alternar_idioma = ComandoAlternarIdioma(
            self.listar_traducoes_use_case, self.obter_use_case
        )
        ...
        while True:
            ...
            if traducoes:
                ...
                for t in traducoes:
                    if t.idioma == idioma_atual and idioma_atual != "original":
                        console.print(
                            f"  ▶ {t.idioma_icone} {t.idioma_nome} - {t.data_traducao} [dim](atual)[/dim]"
                        )
                    else:
            ...
            if cmd == "":
                ...
                cmd_traduzir = ComandoTraduzir(
                    self.traduzir_use_case, self.listar_traducoes_use_case
                )
                try:
