...
from src.application.use_cases.traduzir_documento import TraduzirDocumento  # <-- ADICIONAR ESTA LINHA!
from src.interface.web.routes import documentos, analise, traducoes, estatisticas, admin
...
def create_app(config_path: str = "config.yaml"):
    ...
    analisar_acervo_use_case = AnalisarAcervo(
        repo_doc=repo_doc,
        registry=registry
    )
    
    traduzir_use_case = TraduzirDocumento(
        repo_doc=repo_doc,
        repo_trad=repo_trad,
        registry=registry
    )
