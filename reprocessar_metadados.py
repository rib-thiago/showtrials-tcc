...
def gerar_estatisticas():
    ...
    if 'centro' in colunas:
        ...
        if centros:
            ...
            for centro, total in centros:
                nome = "Leningrad" if centro == "lencenter" else "Moscow" if centro == "moscenter" else centro
                print(f"  • {nome}: {total}")
    ...
    if 'tipo_documento' in colunas:
        ...
        if carta_stats and any(carta_stats):
            ...
            if carta_stats[0]: print(f"  • Cartas: {carta_stats[0]}")
            if carta_stats[1]: print(f"  • Declarações: {carta_stats[1]}")
            if carta_stats[2]: print(f"  • Relatórios: {carta_stats[2]}")
            if carta_stats[3]: print(f"  • Acareações: {carta_stats[3]}")
...
def reprocessar_todos(limite=None):
    ...
    for i, (doc_id, titulo) in enumerate(todos_docs, 1):
        try:
            ...
            print(f"  [{i:3d}] {tipo_icone} ID {doc_id:3d}: {metadados['tipo_descricao']:25} - {pessoa[:30]}")
            processados += 1
